...
class Telegram:
    ...
    def notifyTagChange(self, oldTag, newTag, env, repoName, namespace):
        # Check Proxy enabled
...
""".format(repoName.split("/")[-1], self.cluster, env, oldTag, newTag)
    ...
    def notifySuccess(self, imageName, namespace, startedTime):
        # Check Proxy enabled
        if self.proxy == "true": enableProxy(self.proxyInfo)
        channel = json.loads(self.channel)
        bot_message = """
<b>Service [<code>{}</code>] with new version [<code>{}</code>] has been successfully deployed at [<code>{}</code>].</b>
    ...
    def notifyFailure(self, imageName, namespace):
        # Check Proxy enabled
        if self.proxy == "true": enableProxy(self.proxyInfo)
        channel = json.loads(self.channel)
        bot_message = """
